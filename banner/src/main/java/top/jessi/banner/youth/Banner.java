package top.jessi.banner.youth;
...
import top.jessi.banner.youth.indicator.Indicator;
import top.jessi.banner.youth.listener.OnBannerListener;
import top.jessi.banner.youth.listener.OnPageChangeListener;
import top.jessi.banner.youth.transformer.MZScaleInTransformer;
import top.jessi.banner.youth.transformer.ScaleInTransformer;
import top.jessi.banner.youth.util.BannerLifecycleObserver;
import top.jessi.banner.youth.util.BannerLifecycleObserverAdapter;
import top.jessi.banner.youth.util.BannerUtils;
import top.jessi.banner.youth.util.ScrollSpeedManger;
...
public class Banner<T, BA extends BannerAdapter<T, ? extends RecyclerView.ViewHolder>> extends FrameLayout implements BannerLifecycleObserver {
        ...
        if (hasFocus()) {
            ...
            switch (keyCode) {
                ...
                case KeyEvent.KEYCODE_DPAD_CENTER:
                    /*设置在tv中点击回调*/
                    if (getAdapter() != null && mOnBannerListener != null) {
                        int real = BannerUtils.getRealPosition(isInfiniteLoop(), getCurrentItem(), getRealCount());
                        T data = (T) getAdapter().getData(real);
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            normalWidth = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_normal_width,
                    BannerConfig.INDICATOR_NORMAL_WIDTH);
            selectedWidth = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_selected_width,
                    BannerConfig.INDICATOR_SELECTED_WIDTH);
            normalColor = a.getColor(R.styleable.Banner_banner_indicator_normal_color,
                    BannerConfig.INDICATOR_NORMAL_COLOR);
            selectedColor = a.getColor(R.styleable.Banner_banner_indicator_selected_color,
            ...
            indicatorHeight = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_height,
                    BannerConfig.INDICATOR_HEIGHT);
            indicatorRadius = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_radius,
    ...
    private void setRecyclerViewPadding(int leftItemPadding, int rightItemPadding) {
        ...
        if (getViewPager2().getOrientation() == ViewPager2.ORIENTATION_VERTICAL) {
            recyclerView.setPadding(mViewPager2.getPaddingLeft(), leftItemPadding, mViewPager2.getPaddingRight(),
                    rightItemPadding);
        } else {
            recyclerView.setPadding(leftItemPadding, mViewPager2.getPaddingTop(), rightItemPadding,
                    mViewPager2.getPaddingBottom());
        }
